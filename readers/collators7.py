...
class CollatorWithChainsChatFormat:
    ...
    def __init__(self, text_maxlength, answer_maxlength=25, context_type="triples",
                 **kwargs):
        """
    ...
    def get_contexts(self, example):
        ...
        final_paras=[]
        for i, para in enumerate(context_text_list):
            final_paras.append(("Passage-%d: " % i) + para)
        context_text = "\n".join(final_paras)

        return context_text, context_text_list
    ...
    def get_prompts_chat_format(self, batch):
        ...
        for example in batch:
            if has_contexts:
                context_str, context_text_list = self.get_contexts(example)
        ...
        for example, context_str in zip(batch, context_str_list):

            user_input_template='Given the following information: \n{paras}\nAnswer the following question based on the given information or your internal knowledge with one or few words without the source.\nQuestion: {question}\nAnswer:'
            user_input_text = example["question"]
            if has_contexts:
                user_input_text = user_input_template.format(paras=context_str, question=example["question"])
            else:
        ...
        return prompts, context_text_lists, scores_lists
