...
class ReaderDataset(Dataset):
    ...
    def __init__(self, data_path, n_context=None, chain_key="chains", question_prefix='question:',
                 title_prefix='title:', passage_prefix='context:', **kwargs):
...
class ReaderDatasetWithChains(ReaderDataset):
    ...
    def sort_data(self):
        if self.n_context is None or not self.chain_key in self.data[0] or not 'score' in self.data[0][self.chain_key][
            0]:
            return
    ...
    def __getitem__(self, index):
        ...
        if self.n_context is not None and self.n_context > 0:
            contexts = [{"title": ctx["title"], "sentences": ctx["sentences"], "credibility": ctx["text_truthful_score"]} for ctx in example["ctxs"]]
            chains = []
