...
class CollatorWithChainsChatFormat:
    ...
    def __init__(self, tokenizer, text_maxlength, answer_maxlength=25, context_type="triples", with_scores=False,**kwargs):
        """
    ...
    def get_contexts(self, example):
        ...
        if self.context_type == "all_documents":
            ...
            if self.with_scores:
                all_documents_list = [
                    "title: {}, text: {}\nTruthful score: {}".format(context_item["title"], " ".join(context_item["sentences"]), context_item["credibility"]) for
    ...
    def get_prompts_chat_format(self, batch):
        ...
        for example in batch:
            ...
            if has_contexts:
                if self.with_scores:
                    user_input_text = f'You are an assistant capable of answering questions based on provided passages. Each passage is assigned a truthfulness score (0-10), where a higher score indicates greater credibility. Your answer need to combine multiple passages and their credibility. Given the following information: \n{self.get_contexts(example)}\nAnswer the following question based on the given information or your internal knowledge with one or few words without the source (just output a answer, don\'t output anything else).\nQuestion: {user_input_text}\nthe correct answer is:'
                else:
