...
def retrieve_documents_by_similarity_score(question: str, ctxs: List[Dict], args, ideal_setting: bool = True) -> List[
    Dict]:
    """
...
class Llama3DataProcessor:
    ...
    def create_llama3_prompt(self, question: str, retrieved_docs: List[Dict]) -> List[Dict]:
        ...
        if retrieved_docs:
            ...
            prompt = f'Answer the following question based on your internal knowledge with one or few words without the source.\nQuestion: {question}\nAnswer: '
...
class QwenDataProcessor:
    ...
    def create_qwen_prompt(self, question: str, retrieved_docs: List[Dict]) -> List[Dict]:
        """
        创建符合Qwen格式的prompt
    ...
    def create_qwen_prompt_score(self, question: str, retrieved_docs: List[Dict]) -> List[Dict]:
        """
        创建符合Qwen格式的prompt
...
def parse_llama3_answer(answer):
    """
    解析Llama3生成的答案
...
def evaluate_with_llama3(args, model, tokenizer, data):
    ...
    for i, example in enumerate(tqdm(data, desc="正在评估")):
        ...
        predicted_answer = parse_llama3_answer(generated_text)
...
# ============ Qwen评估函数 ============
def evaluate_with_qwen(args, model, tokenizer, data):
    """
    使用Qwen模型评估检索结果
    ...
    """
    em_scores_list, f1_scores_list = [], []
    processor = QwenDataProcessor(args)
    ...
    for i, example in enumerate(tqdm(data, desc="正在评估")):
        ...
        if args.prompt_based:
            retrieved_docs = retrieve_documents_by_similarity_score(question, ctxs, args)
            messages = processor.create_qwen_prompt_score(question, retrieved_docs)
        else:
            retrieved_docs = retrieve_documents_by_similarity(question, ctxs, args)
            messages = processor.create_qwen_prompt(question, retrieved_docs)
        ...
        generated_ids = outputs[0, model_inputs["input_ids"].shape[1]:].detach().cpu()
        generated_text = tokenizer.decode(generated_ids, skip_special_tokens=True)
        predicted_answer = parse_qwen_answer(generated_text)
...
class CagDataProcessor:
    ...
    def create_prompt(self, question: str, retrieved_docs: List[Dict]) -> List[Dict]:
        """
        创建符合指定格式的prompt
...
'''
        ...
        if retrieved_docs:
            ...
            user_input = f"{few_shot_examples}\nQuestion: {question}\n\nAnswer (one or few words only):"
...
def setup_parser():
    ...
    parser.add_argument("--input_data_file", type=str, default="data/hotpotqa/dev_with_kgs.json",
                        help="输入数据文件路径")
    parser.add_argument("--model_type", type=str, choices=["cag", "llama3"], default="cag",
...
def main():
    ...
    print(f"CAG模型路径: {args.model_path}")
    print("=" * 80)
    ...
    if args.model_type == "llama3":
        ...
        print("步骤3: 开始检索和评估（使用Llama3，基于相似度检索）...")
        metrics = evaluate_with_llama3(args, model, tokenizer, data)
    elif args.model_type == "qwen":
        model, tokenizer = load_qwen_model_tokenizer(args.model_path)
        print("步骤3: 开始检索和评估（使用Qwen1.5-7B，基于相似度检索）...")
        metrics = evaluate_with_qwen(args, model, tokenizer, data)
    else:
        # 加载CAG-7B模型
