...
class Re_Weighting_Strategy:
    ...
    def decode_with_special_attention(self,prompt: str='', paras: list = [], scores: list = [], answer: str = ''):
    ...
    @torch.no_grad()
    def run_RAG_with_attention_weighting(self, prompt: str='', paras: list = [], scores: list = []):
        model_inputs, attention_weight = self.decode_with_special_attention(prompt=prompt,paras=paras, scores=scores)
        registered_hooks = []
        ...
        return (model_inputs["attention_mask"] > 0).sum().item(), output
...
class Find_Best_Heads(Re_Weighting_Strategy):
    ...
    @torch.no_grad()
    def cal_logits(self, prompt: str='', paras: list = [], scores: list = [], right_answer: str = '', wrong_answer: str = 'my name is'):
        answer = wrong_answer
        answer_ids = self.tokenizer([answer], return_tensors="pt")['input_ids'][0, 1:]
        model_inputs, attention_weight = self.decode_with_special_attention(prompt=prompt, paras=paras, scores=scores, answer=answer)
        registered_hooks = []
