...
def evaluate(args, tokenizer, dataloader, model):
    em_scores_list, f1_scores_list, precision_scores_list, recall_scores_list, num_tokens_list,accuracy_list = [], [], [], [], [],[]
    ...
    for batch_index, batch_inputs in tqdm(dataloader, desc="Evaluation", total=len(dataloader)):
        ...
        batch_outputs = model.generate(**batch_inputs, max_new_tokens=args.answer_maxlength, do_sample=False,
                                       temperature=1.0)
        ...
        for i, o in enumerate(batch_generated_token_ids):
            ...
            accuracy_list.append(accuracy(ans, gold))
            # if not accuracy(ans, gold):
            #         print(ans, "\t", gold)
